...
namespace YoutubeDL
{
    class SQLiteDatabase
    {
        ...
        public int Update(String tableName, KeyValuePair<string, object>[] data, String where)
        {
            SQLiteConnection cnn = new SQLiteConnection(dbConnection);
            SQLiteCommand mycommand = new SQLiteCommand(cnn);
            ...
            {
                string param = "@" + val.Key;
                vals.AppendFormat("    ,{0} = {1}\n", val.Key, param);

                mycommand.Parameters.Add(new SQLiteParameter(param, val.Value));
            }
            string setsql = vals.Remove(0, 5).ToString();

            mycommand.CommandText = String.Format("UPDATE {0} SET {1} WHERE {2};", tableName, setsql, where);
